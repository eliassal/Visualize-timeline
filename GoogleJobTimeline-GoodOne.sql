...
-- Date:	2015-06-24
-- https://www.sqlservercentral.com/Forums/1706175/Visualize-the-timeline-of-your-SQL-jobs-using-Google-graph-and-email?PageIndex=7
--****************************************************************************************
set nocount on 
...
declare @subjectWithSQL varchar(150)
declare @query_attachment_filenameFull varchar(50)
DECLARE @datestring varchar(10)
--***************************************************************************************
-- Set variables
--***************************************************************************************
set @StartDT = getdate() - 3  
Print @StartDT
set @EndDT = getdate()
Print @EndDT
...
set @SendMail = 1
set @Emailprofilename = 'SalamAdminOnNet'
set @EmailRecipients = 'salam.elias@free.fr'
set @Show = 'Job/Step'
set @ServerName = @@servername 
set @subjectWithSQL = 'JobTimeline' + ' ' + @ServerName

set @datestring = CONVERT(DATE, GETDATE())
set @query_attachment_filenameFull = 'JobTimeline' +  @ServerName + '-' +  @datestring + '.html'
--***************************************************************************************
...
--NothingToDo: print 'No job runtime info found....'
--***************************************************************************************
-- Send Email - 
--***************************************************************************************
...
if @SendMail = 1 
	execute msdb.dbo.sp_send_dbmail	
		...
		,@subject = @subjectWithSQL --'JobTimeline'
		...
		,@query_attachment_filename= @query_attachment_filenameFull --'JobTimeline.HTML'
